...
function(ecm_target_qml_sources ARG_TARGET)
    ...
    foreach(_file ${ARG_SOURCES})
        # Check if a given file exists, but only for files relative to the
        # current source dir, otherwise things might be too noisy/restrictive.
        set(_path "${_file}")
        set(_in_source_dir FALSE)
        if (NOT IS_ABSOLUTE ${_path})
            string(FIND ${_file} ${CMAKE_BINARY_DIR} _in_binary_dir)
            if (${_in_binary_dir} LESS 0)
                set(_in_source_dir TRUE)
                set(_path "${CMAKE_CURRENT_SOURCE_DIR}/${_file}")
            endif()
        else()
            string(FIND ${_file} ${CMAKE_CURRENT_SOURCE_DIR} _source_dir_found)
            if (_source_dir_found GREATER_EQUAL 0)
                set(_in_source_dir TRUE)
            endif()
        endif()

        if (${_in_source_dir} AND NOT EXISTS ${_path})
