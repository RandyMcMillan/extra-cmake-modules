...
::
  ...
  ecm_generate_export_header(<library_target_name>
      ...
      [USE_VERSION_HEADER [<version_file_name>]] #  Since 5.105
      [VERSION_MACRO_NAME <version_macro_name>] #  Since 5.105
...
``USE_VERSION_HEADER`` defines whether a given header file providing macros
specifying the library version should be included in the generated header
file. The default header file name used will be
``<lowercase_base_name>_version.h`` and can be overwritten by passing
an argument with ``<version_file_name>``. By default angle-brackets are used
...
The macro from the included version header holding the library version is by
default assumed to be ``<uppercase_base_name>_VERSION`` and used in the
generated code for calculating defaults. To specify another name, pass it with
``VERSION_MACRO_NAME`` as ``<version_macro_name>``. Since 5.104.
...
function(ecm_generate_export_header target)
    ...
    if (USE_VERSION_HEADER IN_LIST ARGS_KEYWORDS_MISSING_VALUES)
        set(_version_header "${_lower_base_name}_version.h")
    ...
    if(_version_header)
        if(ARGS_VERSION_MACRO_NAME)
            ...
            set(_version_hexnumber "${_upper_base_name}_VERSION")
        endif()
    else()
